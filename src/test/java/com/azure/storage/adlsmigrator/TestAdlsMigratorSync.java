...
public class TestAdlsMigratorSync {
  ...
  private AdlsMigratorOptions options;
  ...
  public void init() throws Exception {
    ...
    options = new AdlsMigratorOptions(Arrays.asList(source), target);
    options.setSyncFolder(true);
    options.setUseDiff("s1", "s2");
    options.appendToConf(conf);

    conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH, target.toString());
    conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_FINAL_PATH, target.toString());
  }
  ...
  private boolean sync() throws Exception {
    AdlsMigratorSync AdlsMigratorSync = new AdlsMigratorSync(options, conf);
    return AdlsMigratorSync.sync();
  }
  ...
  public void testSync() throws Exception {
    ...
    // the later adlsmigrator since we're copying (s2-s1) to target
    ...
    // do the sync
    Assert.assertTrue(AdlsMigratorSync.sync());
    ...
    CopyListing listing = new SimpleCopyListing(conf, new Credentials(), AdlsMigratorSync);
  ...
  public void testSync9() throws Exception {
    ...
    new AdlsMigrator(conf, OptionsParser.parse(args)).execute();
