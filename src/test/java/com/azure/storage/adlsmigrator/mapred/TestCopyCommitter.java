...
public class TestCopyCommitter {
  ...
  public static void create() throws IOException {
    ...
    config.setLong(AdlsMigratorConstants.CONF_LABEL_TOTAL_BYTES_TO_BE_COPIED, 0);
  ...
  public void createMetaFolder() {
    config.set(AdlsMigratorConstants.CONF_LABEL_META_FOLDER, "/meta");
  ...
  public void testPreserveStatus() {
    ...
    try {
      ...
      sourceBase = TestAdlsMigratorUtils.createTestSetup(fs, sourcePerm);
      targetBase = TestAdlsMigratorUtils.createTestSetup(fs, initialPerm);

      AdlsMigratorOptions options = new AdlsMigratorOptions(Arrays.asList(new Path(sourceBase)),
          new Path("/out"));
      ...
      conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH, targetBase);
      ...
      TestAdlsMigratorUtils.delete(fs, "/tmp1");
      conf.unset(AdlsMigratorConstants.CONF_LABEL_PRESERVE_STATUS);
    }
  ...
  public void testDeleteMissing() {
    ...
    try {
      OutputCommitter committer = new CopyCommitter(null, taskAttemptContext);
      fs = FileSystem.get(conf);
      sourceBase = TestAdlsMigratorUtils.createTestSetup(fs, FsPermission.getDefault());
      targetBase = TestAdlsMigratorUtils.createTestSetup(fs, FsPermission.getDefault());
      String targetBaseAdd = TestAdlsMigratorUtils.createTestSetup(fs, FsPermission.getDefault());
      ...
      conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH, targetBase);
      conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_FINAL_PATH, targetBase);

      committer.commitJob(jobContext);
      if (!TestAdlsMigratorUtils.checkIfFoldersAreInSync(fs, targetBase, sourceBase)) {
        Assert.fail("Source and target folders are not in sync");
      }
      if (!TestAdlsMigratorUtils.checkIfFoldersAreInSync(fs, sourceBase, targetBase)) {
        Assert.fail("Source and target folders are not in sync");
      }
      ...
      TestAdlsMigratorUtils.delete(fs, "/tmp1");
      conf.set(AdlsMigratorConstants.CONF_LABEL_DELETE_MISSING, "false");
    }
  }
  ...
  public void testDeleteMissingFlatInterleavedFiles() {
    ...
    try {
      ...
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/1");
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/3");
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/4");
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/5");
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/7");
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/8");
      TestAdlsMigratorUtils.createFile(fs, sourceBase + "/9");

      TestAdlsMigratorUtils.createFile(fs, targetBase + "/2");
      TestAdlsMigratorUtils.createFile(fs, targetBase + "/4");
      TestAdlsMigratorUtils.createFile(fs, targetBase + "/5");
      TestAdlsMigratorUtils.createFile(fs, targetBase + "/7");
      TestAdlsMigratorUtils.createFile(fs, targetBase + "/9");
      TestAdlsMigratorUtils.createFile(fs, targetBase + "/A");
  ...
  public void testAtomicCommitMissingFinal() {
    ...
    try {
      ...
      conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH, workPath);
      conf.set(AdlsMigratorConstants.CONF_LABEL_TARGET_FINAL_PATH, finalPath);
      conf.setBoolean(AdlsMigratorConstants.CONF_LABEL_ATOMIC_COPY, true);
      ...
      TestAdlsMigratorUtils.delete(fs, workPath);
      TestAdlsMigratorUtils.delete(fs, finalPath);
      conf.setBoolean(AdlsMigratorConstants.CONF_LABEL_ATOMIC_COPY, false);
    }
  }
