...
import com.azure.storage.adlsmigrator.StubContext;
...
public class TestCopyMapper {
  ...
  private static Configuration getConfiguration() throws IOException {
    ...
    configuration.set(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH,
            workPath.toString());
    configuration.set(AdlsMigratorConstants.CONF_LABEL_TARGET_FINAL_PATH,
            workPath.toString());
    configuration.setBoolean(AdlsMigratorOptionSwitch.OVERWRITE.getConfigLabel(),
            false);
    configuration.setBoolean(AdlsMigratorOptionSwitch.SKIP_CRC.getConfigLabel(),
            false);
    configuration.setBoolean(AdlsMigratorOptionSwitch.SYNC_FOLDERS.getConfigLabel(),
            true);
    configuration.set(AdlsMigratorOptionSwitch.PRESERVE_STATUS.getConfigLabel(),
            "br");
    return configuration;
  }
  ...
  public void testCopyWithAppend() throws Exception {
    final FileSystem fs = cluster.getFileSystem();
    // do the first adlsmigrator
    testCopy(false);
    // start appending data to source
    appendSourceData();

    // do the adlsmigrator again with -update and -append option
    CopyMapper copyMapper = new CopyMapper();
    Configuration conf = getConfiguration();
    // set the buffer size to 1/10th the size of the file.
    conf.setInt(AdlsMigratorOptionSwitch.COPY_BUFFER_SIZE.getConfigLabel(),
    ...
    context.getConfiguration().setBoolean(
        AdlsMigratorOptionSwitch.APPEND.getConfigLabel(), true);
    ...
    for (Path path: pathList) {
      ...
      copyMapper.map(new Text(AdlsMigratorUtils.getRelativePath(new Path(SOURCE_PATH), path)),
  ...
  private void testCopy(boolean preserveChecksum) throws Exception {
    ...
    EnumSet<AdlsMigratorOptions.FileAttribute> fileAttributes
            = EnumSet.of(AdlsMigratorOptions.FileAttribute.REPLICATION);
    if (preserveChecksum) {
      fileAttributes.add(AdlsMigratorOptions.FileAttribute.CHECKSUMTYPE);
    }
    configuration.set(AdlsMigratorOptionSwitch.PRESERVE_STATUS.getConfigLabel(),
            AdlsMigratorUtils.packAttributes(fileAttributes));
    ...
    for (Path path : pathList) {
      ...
      copyMapper.map(
          new Text(AdlsMigratorUtils.getRelativePath(new Path(SOURCE_PATH), path)),
  ...
  public void testMakeDirFailure() {
    try {
      ...
      copyMapper.map(new Text(AdlsMigratorUtils.getRelativePath(new Path(SOURCE_PATH),
          pathList.get(0))),
  ...
  public void testPreserve() {
    try {
      ...
      EnumSet<AdlsMigratorOptions.FileAttribute> preserveStatus =
          EnumSet.allOf(AdlsMigratorOptions.FileAttribute.class);
      preserveStatus.remove(AdlsMigratorOptions.FileAttribute.ACL);
      preserveStatus.remove(AdlsMigratorOptions.FileAttribute.XATTR);

      context.getConfiguration().set(AdlsMigratorConstants.CONF_LABEL_PRESERVE_STATUS,
  ...
  public void testSkipCopyNoPerms() {
    try {
      ...
      preserveStatus.remove(AdlsMigratorOptions.FileAttribute.ACL);
      preserveStatus.remove(AdlsMigratorOptions.FileAttribute.XATTR);
      preserveStatus.remove(AdlsMigratorOptions.FileAttribute.TIMES);
  ...
  private void doTestIgnoreFailures(boolean ignoreFailures) {
    try {
      ...
      configuration.setBoolean(
              AdlsMigratorOptionSwitch.IGNORE_FAILURES.getConfigLabel(),ignoreFailures);
      configuration.setBoolean(AdlsMigratorOptionSwitch.OVERWRITE.getConfigLabel(),
              true);
      configuration.setBoolean(AdlsMigratorOptionSwitch.SKIP_CRC.getConfigLabel(),
              true);
  ...
  public void testCopyFailOnBlockSizeDifference() {
    try {
      ...
      Configuration configuration = context.getConfiguration();
      EnumSet<AdlsMigratorOptions.FileAttribute> fileAttributes
          = EnumSet.noneOf(AdlsMigratorOptions.FileAttribute.class);
      configuration.set(AdlsMigratorOptionSwitch.PRESERVE_STATUS.getConfigLabel(),
          AdlsMigratorUtils.packAttributes(fileAttributes));
  ...
  private void testPreserveBlockSizeAndReplicationImpl(boolean preserve){
    try {
      ...
      EnumSet<AdlsMigratorOptions.FileAttribute> fileAttributes
              = EnumSet.noneOf(AdlsMigratorOptions.FileAttribute.class);
      if (preserve) {
        fileAttributes.add(AdlsMigratorOptions.FileAttribute.BLOCKSIZE);
        fileAttributes.add(AdlsMigratorOptions.FileAttribute.REPLICATION);
      }
      configuration.set(AdlsMigratorOptionSwitch.PRESERVE_STATUS.getConfigLabel(),
              AdlsMigratorUtils.packAttributes(fileAttributes));
  ...
  public void testSingleFileCopy() {
    try {
      ...
      copyMapper.map(new Text(AdlsMigratorUtils.getRelativePath(
  ...
  private void testPreserveUserGroupImpl(boolean preserve){
    try {
      ...
      EnumSet<AdlsMigratorOptions.FileAttribute> fileAttributes
              = EnumSet.noneOf(AdlsMigratorOptions.FileAttribute.class);
      if (preserve) {
        fileAttributes.add(AdlsMigratorOptions.FileAttribute.USER);
        fileAttributes.add(AdlsMigratorOptions.FileAttribute.GROUP);
        fileAttributes.add(AdlsMigratorOptions.FileAttribute.PERMISSION);
      }
  ...
  public void testVerboseLogging() throws Exception {
    ...
    context.getConfiguration().setBoolean(
        AdlsMigratorOptionSwitch.VERBOSE_LOG.getConfigLabel(), true);
    copyMapper.setup(context);
