...
/**
 * Tests adlsmigrator in combination with HDFS ACLs.
 */
public class TestAdlsMigratorWithAcls {
  ...
  public void testPreserveAcls() throws Exception {
    assertrunAdlsMigrator(AdlsMigratorConstants.SUCCESS, "/dstPreserveAcls");
  ...
  public void testAclsNotEnabled() throws Exception {
    try {
      restart(false);
      assertrunAdlsMigrator(AdlsMigratorConstants.ACLS_NOT_SUPPORTED, "/dstAclsNotEnabled");
    } finally {
      restart(true);
    }
  }
  ...
  public void testAclsNotImplemented() throws Exception {
    assertrunAdlsMigrator(AdlsMigratorConstants.ACLS_NOT_SUPPORTED,
      "stubfs://dstAclsNotImplemented");
  }

  /**
   * Stub FileSystem implementation used for testing the case of attempting
   * adlsmigrator with ACLs preserved on a file system that does not support ACLs.
  ...
  /**
   * Runs adlsmigrator from /src to specified destination, preserving ACLs.  Asserts
   * expected exit code.
   *
   * @param int exitCode expected exit code
   * @param dst String adlsmigrator destination
   * @throws Exception if there is any error
   */
  private static void assertrunAdlsMigrator(int exitCode, String dst)
      throws Exception {
    AdlsMigrator adlsMigrator = new AdlsMigrator(conf, null);
    assertEquals(exitCode, ToolRunner.run(
      conf, adlsMigrator, new String[] { "-pa", "/src", dst }));
  }
