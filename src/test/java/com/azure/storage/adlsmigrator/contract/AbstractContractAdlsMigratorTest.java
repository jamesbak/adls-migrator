...
package com.azure.storage.adlsmigrator.contract;
...
/**
 * Contract test suite covering a file system's integration with AdlsMigrator.  The
 ...
 * case).
 */
public abstract class AbstractContractAdlsMigratorTest
  ...
  private void deepDirectoryStructure(FileSystem srcFS, Path srcDir,
    ...
    runAdlsMigrator(inputDir, target);
  ...
  private void largeFiles(FileSystem srcFS, Path srcDir, FileSystem dstFS,
    ...
    int fileSizeKb = conf.getInt("scale.test.adlsmigrator.file.size.kb", 10 * 1024);
  ...
  /**
   * Executes AdlsMigrator and asserts that the job finished successfully.
   *
   * @param src source path
   * @param dst destination path
   * @throws Exception if there is a failure
   */
  private void runAdlsMigrator(Path src, Path dst) throws Exception {
    AdlsMigratorOptions options = new AdlsMigratorOptions(Arrays.asList(src), dst);
    Job job = new AdlsMigrator(conf, options).execute();
    assertNotNull("Unexpected null job returned from AdlsMigrator execution.", job);
    assertTrue("AdlsMigrator job did not complete.", job.isComplete());
    assertTrue("AdlsMigrator job did not complete successfully.", job.isSuccessful());
  }
