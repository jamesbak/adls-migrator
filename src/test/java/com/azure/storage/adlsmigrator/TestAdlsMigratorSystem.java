...
public class TestAdlsMigratorSystem {
  private static final Log LOG =
      LogFactory.getLog(TestAdlsMigratorSystem.class);
  ...
  private void testPreserveUserHelper(String testRoot,
    ...
    ToolRunner.run(conf, new AdlsMigrator(), args);
  ...
  // WC: needed because the current adlsmigrator does not create target dirs
  ...
  private void copyAndVerify(final DistributedFileSystem fs,
    ...
    LOG.info("ls before adlsmigrator");
    LOG.info(execCmd(shell, "-lsr", testRoot));

    LOG.info("_____ running adlsmigrator: " + args[0] + " " + args[1]);
    ToolRunner.run(conf, new AdlsMigrator(), args);

    LOG.info("ls after adlsmigrator");
    LOG.info(execCmd(shell, "-lsr", testRoot));
  ...
  private void chunkCopy(FileEntry[] srcFiles) throws Exception {
    ...
    // modify last file and rerun adlsmigrator with -update option
  ...
  @Test
  public void TestAdlsMigratorLargeFile() throws Exception {
    FileEntry[] srcfiles = {
        new FileEntry(SRCDAT, true),
  ...
  public void testSourceRoot() throws Exception {
    ...
    Assert.assertThat(ToolRunner.run(conf, new AdlsMigrator(), args), is(0));
    ...
    Assert.assertThat(ToolRunner.run(conf, new AdlsMigrator(), args2), is(0));
  }
}