...
/**
 * Tests adlsmigrator in combination with HDFS XAttrs.
 */
public class TestAdlsMigratorWithXAttrs {
  ...
  public void testPreserveXAttrs() throws Exception {
    AdlsMigratorTestUtils.assertrunAdlsMigrator(AdlsMigratorConstants.SUCCESS, rootedSrcName,
    ...
    AdlsMigratorTestUtils.assertXAttrs(dstDir1, fs, xAttrs);
    
    // dstSubDir1
    xAttrs.clear();
    xAttrs.put(name1, value1);
    xAttrs.put(name3, new byte[0]);
    AdlsMigratorTestUtils.assertXAttrs(dstSubDir1, fs, xAttrs);
    ...
    xAttrs.put(name2, value2);
    xAttrs.put(name3, new byte[0]);
    AdlsMigratorTestUtils.assertXAttrs(dstFile1, fs, xAttrs);
    
    // dstDir2
    xAttrs.clear();
    xAttrs.put(name2, value2);
    AdlsMigratorTestUtils.assertXAttrs(dstDir2, fs, xAttrs);
    
    // dstFile2
    xAttrs.clear();
    xAttrs.put(name1, value1);
    xAttrs.put(name4, new byte[0]);
    AdlsMigratorTestUtils.assertXAttrs(dstFile2, fs, xAttrs);
    
    // dstFile3
    xAttrs.clear();
    xAttrs.put(name3, new byte[0]);
    xAttrs.put(name4, new byte[0]);
    AdlsMigratorTestUtils.assertXAttrs(dstFile3, fs, xAttrs);
    
    // dstFile4
    xAttrs.clear();
    AdlsMigratorTestUtils.assertXAttrs(dstFile4, fs, xAttrs);
  }
  ...
  public void testXAttrsNotEnabled() throws Exception {
    try {
      restart(false);
      AdlsMigratorTestUtils.assertrunAdlsMigrator(AdlsMigratorConstants.XATTRS_NOT_SUPPORTED,
   ...
   * adlsmigrator with XAttrs preserved on a file system that does not support XAttrs. 
