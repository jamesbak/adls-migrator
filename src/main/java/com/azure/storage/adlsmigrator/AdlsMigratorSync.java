...
import com.azure.storage.adlsmigrator.CopyListing.InvalidInputException;
...
class AdlsMigratorSync {
  ...
  // diff stored there maybe modified instead of copied by the adlsmigrator algorithm
  ...
  AdlsMigratorSync(AdlsMigratorOptions options, Configuration conf) {
   ...
   *  default adlsmigrator if the third condition isn't met.
   */
  private boolean preSyncCheck() throws IOException {
    ...
    if (!(srcFs instanceof DistributedFileSystem) ||
      ...
      throw new IllegalArgumentException("The FileSystems needs to" +
          " be DistributedFileSystem for using snapshot-diff-based adlsmigrator");
    }
  ...
  public boolean sync() throws IOException {
    ...
    try {
      ...
      AdlsMigrator.LOG.warn("Failed to use snapshot diff for adlsmigrator", e);
      return false;
    } finally {
  ...
  private boolean getAllDiffs() throws IOException {
    ...
    try {
      ...
      AdlsMigrator.LOG.warn("Failed to compute snapshot diff on " + ssDir, e);
    }
    this.diffMap = null;
    return false;
  }
  ...
  private Path createTargetTmpDir(DistributedFileSystem targetFs,
    ...
    final Path tmp = new Path(targetDir,
        AdlsMigratorConstants.HDFS_ADLSMIGRATOR_DIFF_DIRECTORY_NAME + AdlsMigrator.rand.nextInt());
  ...
  private void deleteTargetTmpDir(DistributedFileSystem targetFs,
    ...
    try {
      ...
      AdlsMigrator.LOG.error("Unable to cleanup tmp dir: " + tmpDir, e);
    }
  }
  ...
  private boolean checkNoChange(DistributedFileSystem fs, Path path) {
    try {
      ...
      if (!targetDiff.getDiffList().isEmpty()) {
        AdlsMigrator.LOG.warn("The target has been modified since snapshot "
      ...
      AdlsMigrator.LOG.warn("Failed to compute snapshot diff on " + path
