...
/**
 * DynamicInputFormat implements the "Worker pattern" for AdlsMigrator.
...
public class DynamicInputFormat<K, V> extends InputFormat<K, V> {
  ...
  private List<InputSplit> createSplits(JobContext jobContext,
                                        List<DynamicInputChunk<K, V>> chunks)
          throws IOException {
    ...
    AdlsMigratorUtils.publish(jobContext.getConfiguration(),
  ...
  private List<DynamicInputChunk<K, V>> splitCopyListingIntoChunksWithShuffle
    ...
    int numEntriesPerChunk = (int)Math.ceil((float)numRecords
                                          /(splitRatio * numMaps));
    AdlsMigratorUtils.publish(context.getConfiguration(),
    ...
    List<DynamicInputChunk<K, V>> openChunks
                  = new ArrayList<DynamicInputChunk<K, V>>();
    
    List<DynamicInputChunk<K, V>> chunksFinal = new ArrayList<DynamicInputChunk<K, V>>();
  ...
  private static <K, V> void closeAll(List<DynamicInputChunk<K, V>> chunks) {
    for (DynamicInputChunk<K, V> chunk: chunks)
      chunk.close();
  }

  private List<DynamicInputChunk<K, V>> createChunks(int chunkCount,
      int nChunksTotal, int nChunksOpenAtOnce)
      throws IOException {
    List<DynamicInputChunk<K, V>> chunks = new ArrayList<DynamicInputChunk<K, V>>();
  ...
  private DynamicInputChunk<K, V> createChunk(int chunkId)
                                              throws IOException {
  ...
  private static Path getListingFilePath(Configuration configuration) {
    String listingFilePathString = configuration.get(
            AdlsMigratorConstants.CONF_LABEL_LISTING_FILE_PATH, "");
  ...
  private static int getNumberOfRecords(Configuration configuration) {
    return AdlsMigratorUtils.getInt(configuration,
                              AdlsMigratorConstants.CONF_LABEL_TOTAL_NUMBER_OF_RECORDS);
  }
  ...
  private static int getMaxChunksTolerable(Configuration conf) {
    int maxChunksTolerable = conf.getInt(
        AdlsMigratorConstants.CONF_LABEL_MAX_CHUNKS_TOLERABLE,
        AdlsMigratorConstants.MAX_CHUNKS_TOLERABLE_DEFAULT);
    if (maxChunksTolerable <= 0) {
      LOG.warn(AdlsMigratorConstants.CONF_LABEL_MAX_CHUNKS_TOLERABLE +
          " should be positive. Fall back to default value: "
          + AdlsMigratorConstants.MAX_CHUNKS_TOLERABLE_DEFAULT);
      maxChunksTolerable = AdlsMigratorConstants.MAX_CHUNKS_TOLERABLE_DEFAULT;
    }
  ...
  private static int getMaxChunksIdeal(Configuration conf) {
    int maxChunksIdeal = conf.getInt(
        AdlsMigratorConstants.CONF_LABEL_MAX_CHUNKS_IDEAL,
        AdlsMigratorConstants.MAX_CHUNKS_IDEAL_DEFAULT);
    if (maxChunksIdeal <= 0) {
      LOG.warn(AdlsMigratorConstants.CONF_LABEL_MAX_CHUNKS_IDEAL +
          " should be positive. Fall back to default value: "
          + AdlsMigratorConstants.MAX_CHUNKS_IDEAL_DEFAULT);
      maxChunksIdeal = AdlsMigratorConstants.MAX_CHUNKS_IDEAL_DEFAULT;
    }
  ...
  private static int getMinRecordsPerChunk(Configuration conf) {
    int minRecordsPerChunk = conf.getInt(
        AdlsMigratorConstants.CONF_LABEL_MIN_RECORDS_PER_CHUNK,
        AdlsMigratorConstants.MIN_RECORDS_PER_CHUNK_DEFAULT);
    if (minRecordsPerChunk <= 0) {
      LOG.warn(AdlsMigratorConstants.CONF_LABEL_MIN_RECORDS_PER_CHUNK +
          " should be positive. Fall back to default value: "
          + AdlsMigratorConstants.MIN_RECORDS_PER_CHUNK_DEFAULT);
      minRecordsPerChunk = AdlsMigratorConstants.MIN_RECORDS_PER_CHUNK_DEFAULT;
    }
  ...
  private static int getSplitRatio(Configuration conf) {
    int splitRatio = conf.getInt(
        AdlsMigratorConstants.CONF_LABEL_SPLIT_RATIO,
        AdlsMigratorConstants.SPLIT_RATIO_DEFAULT);
    if (splitRatio <= 0) {
      LOG.warn(AdlsMigratorConstants.CONF_LABEL_SPLIT_RATIO +
          " should be positive. Fall back to default value: "
          + AdlsMigratorConstants.SPLIT_RATIO_DEFAULT);
      splitRatio = AdlsMigratorConstants.SPLIT_RATIO_DEFAULT;
    }
    return splitRatio;
  }
