...
public class CopyCommitter extends FileOutputCommitter {
  ...
  public CopyCommitter(Path outputPath, TaskAttemptContext context) throws IOException {
    ...
    blocksPerChunk = context.getConfiguration().getInt(
        AdlsMigratorOptionSwitch.BLOCKS_PER_CHUNK.getConfigLabel(), 0);
  ...
  public void commitJob(JobContext jobContext) throws IOException {
    ...
    syncFolder = conf.getBoolean(AdlsMigratorConstants.CONF_LABEL_SYNC_FOLDERS, false);
    overwrite = conf.getBoolean(AdlsMigratorConstants.CONF_LABEL_OVERWRITE, false);
    targetPathExists = conf.getBoolean(
        AdlsMigratorConstants.CONF_LABEL_TARGET_PATH_EXISTS, true);
    ignoreFailures = conf.getBoolean(
        AdlsMigratorOptionSwitch.IGNORE_FAILURES.getConfigLabel(), false);
    ...
    String attributes = conf.get(AdlsMigratorConstants.CONF_LABEL_PRESERVE_STATUS);
    final boolean preserveRawXattrs =
        conf.getBoolean(AdlsMigratorConstants.CONF_LABEL_PRESERVE_RAWXATTRS, false);
    if ((attributes != null && !attributes.isEmpty()) || preserveRawXattrs) {
      preserveFileAttributesForDirectories(conf);
    }

    try {
      if (conf.getBoolean(AdlsMigratorConstants.CONF_LABEL_DELETE_MISSING, false)) {
        deleteMissing(conf);
      } else if (conf.getBoolean(AdlsMigratorConstants.CONF_LABEL_ATOMIC_COPY, false)) {
        commitData(conf);
      }
  ...
  private void cleanupTempFiles(JobContext context) {
    try {
      ...
      Path targetWorkPath = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH));
  ...
  private void deleteAttemptTempFiles(Path targetWorkPath,
    ...
    FileStatus[] tempFiles = targetFS.globStatus(
        new Path(targetWorkPath, ".adlsmigrator.tmp." + jobId.replaceAll("job","attempt") + "*"));
  ...
  private void concatFileChunks(Configuration conf) throws IOException {
    ...
    String spath = conf.get(AdlsMigratorConstants.CONF_LABEL_LISTING_FILE_PATH);
    ...
    Path targetRoot =
        new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH));
    ...
    try {
      ...
      while (sourceReader.next(srcRelPath, srcFileStatus)) {
        ...
        Path targetFileChunkPath =
            AdlsMigratorUtils.getSplitChunkPath(targetFile, srcFileStatus);
  ...
  private void preserveFileAttributesForDirectories(Configuration conf)
      throws IOException {
    String attrSymbols = conf.get(AdlsMigratorConstants.CONF_LABEL_PRESERVE_STATUS);
    final boolean syncOrOverwrite = syncFolder || overwrite;

    LOG.info("About to preserve attributes: " + attrSymbols);

    EnumSet<FileAttribute> attributes = AdlsMigratorUtils.unpackAttributes(attrSymbols);
    final boolean preserveRawXattrs =
        conf.getBoolean(AdlsMigratorConstants.CONF_LABEL_PRESERVE_RAWXATTRS, false);

    Path sourceListing = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_LISTING_FILE_PATH));
    FileSystem clusterFS = sourceListing.getFileSystem(conf);
    SequenceFile.Reader sourceReader = new SequenceFile.Reader(conf,
                                      SequenceFile.Reader.file(sourceListing));
    long totalLen = clusterFS.getFileStatus(sourceListing).getLen();

    Path targetRoot = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH));
    ...
    try {
      ...
      while (sourceReader.next(srcRelPath, srcFileStatus)) {
        ...
        AdlsMigratorUtils.preserve(targetFS, targetFile, srcFileStatus, attributes,
            preserveRawXattrs);
  ...
  private void deleteMissing(Configuration conf) throws IOException {
    ...
    Path sourceListing = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_LISTING_FILE_PATH));
    FileSystem clusterFS = sourceListing.getFileSystem(conf);
    Path sortedSourceListing = AdlsMigratorUtils.sortListing(clusterFS, conf, sourceListing);
    ...
    Path targetFinalPath = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_TARGET_FINAL_PATH));
    targets.add(targetFinalPath);
    Path resultNonePath = Path.getPathWithoutSchemeAndAuthority(targetFinalPath)
        .toString().startsWith(AdlsMigratorConstants.HDFS_RESERVED_RAW_DIRECTORY_NAME)
        ? AdlsMigratorConstants.RAW_NONE_PATH : AdlsMigratorConstants.NONE_PATH;
    AdlsMigratorOptions options = new AdlsMigratorOptions(targets, resultNonePath);
    //
    ...
    Path sortedTargetListing = AdlsMigratorUtils.sortListing(clusterFS, conf, targetListing);
  ...
  private void commitData(Configuration conf) throws IOException {
    ...
    Path workDir = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_TARGET_WORK_PATH));
    Path finalDir = new Path(conf.get(AdlsMigratorConstants.CONF_LABEL_TARGET_FINAL_PATH));
