...
class DynamicInputChunkContext<K, V> {
  ...
  public DynamicInputChunk<K, V> acquire(TaskAttemptContext taskAttemptContext)
    ...
    if (fs.exists(acquiredFilePath)) {
      ...
      return new DynamicInputChunk<K, V>(acquiredFilePath, taskAttemptContext, this);
    }
    ...
    for (FileStatus chunkFile : getListOfChunkFiles()) {
      if (fs.rename(chunkFile.getPath(), acquiredFilePath)) {
        ...
        return new DynamicInputChunk<K, V>(acquiredFilePath, taskAttemptContext,
            this);
      }
    }
    return null;
  ...
  public DynamicInputChunk<K, V> createChunkForWrite(String chunkId)
      throws IOException {
    return new DynamicInputChunk<K, V>(chunkId, this);
  }
