...
class DynamicInputChunk<K, V> {
  ...
  DynamicInputChunk(String chunkId, DynamicInputChunkContext<K, V> chunkContext)
                                                      throws IOException {
  ...
  public DynamicInputChunk(Path chunkFilePath,
      ...
      DynamicInputChunkContext<K, V> chunkContext) throws IOException,
  ...
  private void openForRead(TaskAttemptContext taskAttemptContext)
    ...
    reader.initialize(new FileSplit(chunkFilePath, 0,
            AdlsMigratorUtils.getFileSize(chunkFilePath,
